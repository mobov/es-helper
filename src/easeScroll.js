 ...
 * @param transition
 */
export function scrollToYView($scroller, $viewer, justify = 0, duration = 500, transition = 'sineaseOut') {
 ...
 * @param transition
 */
export function scrollToY($scroller, justify = 0, duration = 500, transition = 'sineaseOut') {
  if (!$scroller || justify === 0) { return }
	if (!TRANS_FUNCS.includes(transition)) { return }
  const scroll = justify === 'start' ? -$scroller.scrollTop
